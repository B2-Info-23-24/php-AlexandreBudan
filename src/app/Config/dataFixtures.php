...
function makeAddress(PDO $db, Faker\Generator $faker)
{
...
function makeCar(PDO $db, Faker\Generator $faker)
{
    ...
    for ($i = 0; $i < rand(2, 4); $i++) {
        ...
        makeReservation($db, $faker, $id, $userId, $price, $beginDate, $endDate);
        makeUnvailableDate($db, $id, $beginningString, $endingString);
        $beginDate->modify('+' . strval(rand(3, 7)) . ' days');
    }

    for ($i = 0; $i < rand(2, 5); $i++) {
        $userId = $db->query("SELECT id FROM User ORDER BY rand() LIMIT 1")->fetchColumn();
        makeFavori($db, $id, $userId);
    }
}

function makeUnvailableDate(PDO $db, int $carId, string $beginningDate, string $endingDate)
{
    try {
...
function makeFavori(PDO $db, int $carId, int $userId)
{
    try {
...
function makeReservation(PDO $db, Faker\Generator $faker, int $carId, int $userId, float $price, DateTime $beginningDate, DateTime $endingDate)
{
    ...
    makePilote($db, $faker, $reservationId);
    makeOpinion($db, $faker, $carId, $userId, $reservationId);
}

function makePilote(PDO $db, Faker\Generator $faker, int $reservationId)
{
    $firstName = $faker->firstName;
    $lastName = $faker->lastName;
    $email =  $firstName . "." . $lastName . "@gmail.com";
...
function makeOpinion(PDO $db, Faker\Generator $faker, int $carId, int $userId, int $reservationId)
{
